...
AsyncFileClient.maxFileGenerationHoursDuration = 24;
AsyncFileClient.getStatus = (file) => {
    if (file.filename) {
        return AsyncFile_1.AsyncFileStatus.Successed;
    }
    const creationDate = new Date(file.creationDate);
    return new Date().getTime() > date_fns_1.addHours(creationDate, AsyncFileClient.maxFileGenerationHoursDuration).getTime()
