...
class PublicStatsClient {
    constructor(client) {
        ...
        this.getReportCount = (filters) => {
            const qs = filters ? (0, rxjs_1.pipe)(ReportsClient_1.cleanReportFilter, ReportsClient_1.reportFilter2QueryString, helper_1.cleanObject)(filters) : undefined;
...
class PublicStatsPercentageClient {
    constructor(client) {
        ...
        this.getPercentByStatus = async ({ companyId, status, baseStatus, start, end, }) => {
            ...
            return { value: (0, helper_1.roundValue)(+count.value / +baseCount.value * 100) };
        };
        this.getReportForwardedToPro = (companyId) => {
            return this.getPercentByStatus({
                ...
                end: (0, date_fns_1.subDays)(new Date(), this.delayBeforeCountingToWaitForProResponseInDays)
            });
        };
    ...
    constructor(client) {
        ...
        this.getPercent = (curve, baseCurve) => {
            ...
            for (let i = 0; i < curve.length; i++) {
                ...
                res[i] = {
                    count: (0, helper_1.roundValue)(curve[i].count / baseCurve[i].count * 100),
                    date: curve[i].date,
                };
            }
